...
Using Claude CLI inside vterm buffer

#+begin_src emacs-lisp
  
  (defun claude-chat ()
    (interactive)
    (let ((buf (get-buffer-create "*claude-chat*")))
  ...
  (global-set-key (kbd "C-c k c") 'claude)

  (torresmo/leader-keys
    "ac" '(:ignore t :which-key "Claude")
    "acc" '(claude-chat :which-key "Chat")
    )

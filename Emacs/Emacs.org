...
#+begin_src emacs-lisp
  ...
  (defun torresmo/open-packages-file()
    (interactive)
    (find-file (expand-file-name "~/Workspaces/Personal/dotfiles/packages.yaml")))
  ...
  (torresmo/leader-keys
      ...
      "cp" '(torresmo/open-packages-file :which-key "Packages")
...
#+begin_src emacs-lisp
  ...
  (torresmo/leader-keys 
    ...
    "bd" '((lambda () (interactive) (kill-buffer (current-buffer))) :which-key "Kill Buffer")
...
We can narrow a region to restrict editing the buffer in the selected region
...
#+begin_src emacs-lisp
  ...
  ;; Single monitor
  ;; (defvar torresmo/default-font-size 150)
  ;; (defvar torresmo/default-variable-font-size 150)
...
Use  =org-download= to paste images from clipboard and put in a directory with the org document

#+begin_src emacs-lisp

  (defun torresmo/org-download-image-dir ()
    (when buffer-file-name 
      (let* ((parent-dir (file-name-directory (directory-file-name (file-name-directory buffer-file-name))))
             (image-dir (expand-file-name "images" parent-dir)))
  ...
  (use-package org-download
    ...
    (advice-add 'org-download--dir :override #'torresmo/org-download-image-dir)
...
#+begin_src emacs-lisp
  ...
  (torresmo/leader-keys
    ...
    "op"  '(org-download-clipboard :which-key "Paste image")
...
Configure feature to move text up or down. 

#+begin_src emacs-lisp

  (use-package move-text
    :bind (("C-k" . move-text-up)
           ("C-j" . move-text-down)))
...
Configure keys to control multiple cursors

#+begin_src emacs-lisp

  (use-package multiple-cursors
    :bind (("C-x j i" . mc/edit-lines)
           ("C-x j j" . mc/mark-next-like-this) 
           ("C-x j k" . mc/mark-previous-like-this) 
           ("C-x j a" . mc/mark-all-like-this)))
...
AI integrations using cool libraries

*** Gptel

For AI backend we are using [[https://github.com/karthink/gptel][gptel]], a awesome LLM client to Emacs.
...
#+begin_src emacs-lisp
  ...
  (use-package gptel
    :config
    (setq
     gptel-model "claude-sonnet-4-20250929"
...
For agentic programming we are using [[https://github.com/tninja/aider.el][Aider]]

#+begin_src emacs-lisp
  
  (use-package aider
    :config
    (setq aider-args '("--model" "anthropic/claude-sonnet-4-5-20250929"))
    (let ((api-key (auth-source-pick-first-password :host "api.anthropic.com")))
    (if api-key
  ...
  (torresmo/leader-keys
    "ai" '(:ignore t :which-key "Aider")
    "aic" '(aider-run-aider :which-key "Chat")
    "aim" '(aider-transient-menu :which-key "Menu")
    )
...
Using Kiro CLI inside vterm buffer

#+begin_src emacs-lisp
  
  (defun kiro-chat ()
    (interactive)
    (let ((buf (get-buffer-create "*kiro-chat*")))
      (switch-to-buffer buf)
      (unless (get-buffer-process buf)
        (vterm buf)
        (vterm-send-string "kiro-cli")
        (vterm-send-return))))

  (global-set-key (kbd "C-c k c") 'kiro-chat)

  (torresmo/leader-keys
    "ak" '(:ignore t :which-key "Kiro")
    "akc" '(kiro-chat :which-key "Chat")
    )
...
When launching Emacs from a GUI ir doesn't start a login shell. This means shell configuration files and important environment variables (like those added by =asdf=) may not be available inside Emacs.
The [[https://github.com/purcell/exec-path-from-shell][exec-path-from-shell]] solves this invoking a non-interactive shell and importing the environment variables into Emacs.
...
Basic configuration for Python language
...
Basic configuration for Powershell language
...
#+begin_src emacs-lisp
  ...
  (use-package powershell
    ...
    (setq powershell-power-shell-exe "pwsh"))
